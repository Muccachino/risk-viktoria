...
public class Board {
    ...
    public Board(String boardChoice) {
        ...
        if(boardChoice.equals("board1")) {
            ...
            for (int i = 1; i <= 24; i++) {
                territories.put("Territory " + i, new Territory("Territory " + i, i, TerritoryNames.territoryNames1[i-1]));
            }

            for (int i = 0; i < 4; i++) {
                continents[i] = new Continent(TerritoryNames.continentNames1[i]);
                for (int j = 0; j < 6; j++) {
        ...
        if(boardChoice.equals("board2")) {
            ...
            for (int i = 0; i < continents.length; i++) {
                continents[i] = new Continent(TerritoryNames.continentNames2[i]);
            }
            for (int i = 1; i <= 34; i++) {
                Territory territory = new Territory("Territory " + i, i, TerritoryNames.territoryNames2[i-1]);
                if (i <= 7) {
        ...
        if(boardChoice.equals("board3")) {
            ...
            for (int i = 0; i < continents.length; i++) {
                continents[i] = new Continent(TerritoryNames.continentNames3[i]);
            }
            for (int i = 1; i <= 42; i++) {
                Territory territory = new Territory("Territory " + i, i, TerritoryNames.territoryNames3[i-1]);
                if (i <= 9) {
                    continents[0].addTerritory(territory);
                    territory.setContinent(continents[0].getName());
                }
